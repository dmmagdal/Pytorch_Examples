...
class Diffusion:
	...
	def prepare_noise_schedule(self):
		# Initialize linear beta (noise) schedule.
	...
	def noise_images(self, x, t):
		...
		return (
			sqrt_alpha_hat * x + sqrt_one_minus_alpha_hat * epsilon, 
			epsilon
		)
...
def train(args):
	...
	for epoch in range(args.epochs):
		...
		for i, (images, _) in enumerate(pbar):
			...
			logger.add_scalar(
				"MSE", loss.item(), global_step=epoch * l + i
			)
...
def launch():
	...
	args.batch_size = 12
	args.image_size = 64
	args.dataset_path = "./datasets/landscape_img_folder"
...
if __name__ == '__main__':
	...
	plt.imshow(
		torch.cat(
			[torch.cat([i for i in x.cpu()], dim=-1),], dim=-2
		).permute(1, 2, 0).cpu()
	)
	plt.show()