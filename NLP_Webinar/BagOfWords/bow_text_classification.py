...
# develop/1_BoW_text_classification.ipynb
# Source (dataset): https://www.kaggle.com/lakshmi25npathi/imdb-
...
def main():
	...
	print(pd.read_csv(DATA_PATH).sample(5))

	# Bag of words vector for ["the", "gray", "cat", "say", "on",
	# "the", "gray", "mat"] is [0, 1, 1, 2, 2, 1, 0, 1].
	...
	class Sequences(Dataset):
		def __init__(self, path):
			...
			self.vectorizer = CountVectorizer(
				stop_words='english', max_df=0.99, min_df=0.005
			) 
			self.sequences = self.vectorizer.fit_transform(
				df.review.tolist()
			) # Turn data into a bag of words
			#self.labels = df.label.tolist()
			intLabels = []
			for i in df.label.tolist():
				value = 1 if i == "positive" else 0
				intLabels.append(value)
			self.labels = intLabels # Target we want to predict
			self.token2idx = self.vectorizer.vocabulary_
			self.idx2token = {
				idx: token for idx, token in self.token2idx.items()
			}
	...
	class BagOfWordsClassifier(nn.Module):
		def __init__(self, vocab_size, hidden1, hidden2):
			...
			self.fc1 = nn.Linear(vocab_size, hidden1) # Input layer
			self.fc2 = nn.Linear(hidden1, hidden2) # Hidden layer
			self.fc3 = nn.Linear(hidden2, 1) # Output layer
	...
	# Initialize the bag of words classifier neural network model.
	model = BagOfWordsClassifier(
		len(dataset.token2idx), 128, 64
	)
	print(model)

	# Initalize the model loss and optimizer.
	...
	for epoch in range(10): # Train for 10 epochs
		...
		for inputs, target in progress_bar:
			# Zero out gradients for this run.
			model.zero_grad() 
			...
			# Move the optimizer forward.
			optimizer.step()

			progress_bar.set_description(f"Loss: {loss.item():.3f}")

			# Append loss to losses. Increment steps.
			losses.append(loss.item())
			total += 1

		# Calculate epoch loss and append that to the total training
		# losses.
	...
	# Create a function to predict sentiment of a given text.
	def predict_sentiment(text):
		model.eval() # Model in evaluation mode
		with torch.no_grad():
			test_vector = torch.LongTensor(
				dataset.vectorizer.transform([text]).toarray()
			)
	...
	# Analyze reviews for "Cool Cat Saves the Kids".
	test_text = """
