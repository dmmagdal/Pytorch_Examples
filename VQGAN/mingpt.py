...
class CausalSelfAttention(nn.Module):
	...
	def forward(self, x, layer_past=None):
		...
		k = self.key(x).view(B, T, n_head, C // self.n_head)\
			.transpose(1, 2)  # (B, nh, T, hs))
		q = self.query(x).view(B, T, n_head, C // self.n_head)\
			.transpose(1, 2)  # (B, nh, T, hs))
		v = self.value(x).view(B, T, n_head, C // self.n_head)\
			.transpose(1, 2)  # (B, nh, T, hs))
...
class GPT(nn.Module):
	...
	def __init__(self, vocab_size, block_size, n_layer=12, n_head=8, 
			n_embd=256, embd_pdrop=0.0, resid_pdrop=0.0, attn_pdrop=0.0,
			n_unmasked=0):
		super().__init__()
		config = GPTConfig(
			vocab_size=vocab_size, block_size=block_size,
			embd_pdrop=embd_pdrop, resid_pdrop=resid_pdrop,
			attn_pdrop=attn_pdrop, n_layer=n_layer, n_head=n_head,
			n_embd=n_embd, n_unmasked=n_unmasked,
		)

		# input embedding stem.
		self.tok_emb = nn.Embedding(config.vocab_size, config.n_embd)
		self.tok_emb = nn.Parameter(
	...
	def forward(self, idx, embeddings=None):
		...
		if embeddings is not None: # prepend explicit embeddings
			token_embeddings = torch.cat(
				(embeddings, token_embeddings), dim=1
			)
